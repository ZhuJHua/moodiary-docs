...
对话模型始终来自你配置的服务商——想不依赖第三方云服务，可以自建 Ollama、LM Studio 等本地推理服务，内容不出你的设备（见上文"本地服务"）。

另有端侧小模型负责[心情建议](../guide/assistant#2-端侧模型)与[语义搜索](../guide/organize#语义搜索)，在设置中下载后即可完全离线使用，无需任何密钥。
